...
export default class doDeposit extends React.Component {
  state = {
    ID: null,
    transaction_type: '',
  ...
  handleSubmit = event => {
    ...
    axios.post(`http://localhost:8084/guest/deposit`, {ID: parseInt(this.state.ID,10), transaction_type: this.state.transaction_type, transaction_description: this.state.transaction_description, sender: this.state.sender, amount: parseInt(this.state.amount), recipient: this.state.recipient, timestamp:this.state.timestamp})
      .then(res => {
        console.log(res);
